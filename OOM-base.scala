...
object firstdemo {
  ...
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("springleaf")//.setMaster("local") //remove local
...
//    spark.driver.memory                         8G
    ...
    val lines = sc.textFile("/Users/xilaizhang/Desktop/text.txt")
    val result = lines.map {
      line => (langIndep(line), 2)
    }
      .groupByKey(1)
      .filter {
        case (_, vs) => vs.size > 1
      }
      .saveAsTextFile("/Users/xilaizhang/Desktop/output")
